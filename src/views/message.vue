<template>
    <div>
        <nv-head page-type="消息" :fix-head="true" :show-menu="showMenu" :need-add="true" :message-count="no_read_len">
        </nv-head>
        <div class="page">
            <ul class="tabs">
                <li class="item br" :class='{"selected": selectItem === 2}' @click="changeItem(2)">
                    已读消息
                </li>
...
export default {
    ...
    methods: {
      ...
      markall() {
        $.post('https://cnodejs.org/api/v1/message/mark_all', { accesstoken: this.userInfo.token }, (d) => {
          if (d && d.success) {
